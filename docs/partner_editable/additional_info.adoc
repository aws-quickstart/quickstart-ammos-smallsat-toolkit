...
//TODO Andrew, I've removed "Kibana" and swapped in "OpenSearch" for "Elasticsearch" or "Amazon ES" throughout the doc. 
...
IMPORTANT: The OpenSearch Service domain uses an open-access policy with access controlled by an EC2 security group. For more security, use fine-grained access control or modify the access policy to specify IAM users or roles.
