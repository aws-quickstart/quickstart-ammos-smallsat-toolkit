...
//TODO Andrew, I get that you prefer not add a pointer to the deployment guide's testing and post-deployment info in the "Outputs" tab. Since people may not have the guide on their radar when they've freshly completed the deployment, though, it seems like a good idea, and it's our standard practice. What do you see as the downside for this guide or in general?
...
//TODO Andrew, FYI, I've removed "Kibana" and swapped in "OpenSearch" for "Elasticsearch" or "Amazon ES" throughout the doc, including the parameter descriptions (in the .yaml templates), but I didn't touch the parameter names. 
