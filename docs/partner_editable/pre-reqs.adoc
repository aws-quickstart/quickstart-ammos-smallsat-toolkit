...
During deployment, the Quick Start retrieves the whole set configuration files from the S3 bucket and places them in the `/home/ec2-user/AIT-Core/config` directory on the AIT EC2 instance.

//TODO Andrew, The above path doesn't seem right. The files outside the S3 "config" folder wouldn't move into the EC2 "config" folder, would they?
